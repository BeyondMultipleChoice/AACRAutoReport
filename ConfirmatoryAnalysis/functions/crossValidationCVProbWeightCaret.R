...
#modelsUsed <- c("SVM","SLDA","BOOSTING","BAGGING","RF","GLMNET","TREE","MAXENT") #list of classifier algorithms to use, RTextTools

#modelsUsed <- c("LogitBoost","plr") #first set of caret models

#modelsUsed <- c("rf","treebag","AdaBag") #second set of caret models

# modelsUsed <- c("nb","tanSearch","awtan","manb","vbmpRadial") # third set of caret models
#modelsUsed <- c("tanSearch","awtan","manb","vbmpRadial") # third set of caret models #doesn't work

#modelsUsed <- c("glmnet","rpart") #works
#modelsUsed <- c("RFlda","GFS.GCCL") #doesn't work
#modelsUsed <- c("gamLoess","hdrda") #hdrda works

#modelsUsed <- c("stepLDA","svmLinear3")#,"mlp")

#modelsUsed <- c("svmRadial","svmPoly")

#modelsUsed <- c("rmda","smda")

modelsUsed <- c("tan","evtree")
...
for(scoreColumn in scoreColumnVector){
        ...
        scoreLevels <- levels(score)

        modColIndex <- seq(nlevels(score)+1,length(modelsUsed)*(nlevels(score)+1),nlevels(score)+1)
        ...
        for(i in 1:nFolds){
                ...
                dmTrain <- as.matrix(dmTrain)
                dmTest <- as.matrix(dmTest)
                ...
                #train models
                set.seed(37811)
                models <- tryTrainModels2(dmTrain,score[-testFolds[[i]]],algorithms=modelsUsed) #error handling
                
                #classify models
                #predicts <- classify_models(contTest,models)
                predicts <- predictFromModels(models,dmTest)
                ...
                # 
                predDF <- rbindlist(list(predDF,do.call(cbind,predicts)),fill=TRUE)
                # 
        ...
        save(predDF,file=file.path("/home/msteele/AACR/caretTesting",paste0("IndAlgScoring_B",scoreColumn,"_",Sys.time(),".Rdata"))) # save individual model scoring for examination
        ...
        for(i in 1:nFolds){
                ...
                cvR <- lapply(predDF[-testFolds[[i]],modColIndex],function(x) cvRates(x,score[-testFolds[[i]]],scoreLevels)) #returns columnwise TN,FN,FP,TP
                ...
                #cpositiveCodes <- sapply(seq_along(modColIndex),function(x) ifelse(predDF[testFolds[[i]],modColIndex[x]]==scoreLevels[2],1,0))
                ...
                #predDF[,1:2]*cvR[[1]][predDF[,3],]
                
                #numLevelsByBin <- sapply(modelsUsed,function(x) length(grep(x,names(predDF)))-1)
                predScoreColumn <- c(0,grep("predictedScore",names(predDF))) # make array with column indexs of score columns, plus leading zero
                # multiply prediction probabilities with conditional CV metrics
                tmpPred <- lapply(seq_along(modelsUsed),
                                  function(x) predDF[testFolds[[i]],(predScoreColumn[x]+1):(predScoreColumn[x+1]-1)]*cvR[[x]][predDF[testFolds[[i]],predScoreColumn[x+1]],]) 
                tmpPred <- Reduce('+',tmpPred)
                # find maximum weighting factor, and select predicted score
                tmpPred <- apply(tmpPred,1,which.max)
                
                tmpProb <- Reduce('+',lapply(seq_along(modelsUsed),
                                  function(x) predDF[testFolds[[i]],(predScoreColumn[x]+1):(predScoreColumn[x+1]-1)]))/length(modelsUsed)
                tmpProb <- sapply(seq(tmpPred),function(x) tmpProb[x,tmpPred[x]])
                
                tmpPred <- scoreLevels[tmpPred]
