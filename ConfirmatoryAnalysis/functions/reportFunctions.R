...
makeCooccurFreqPlot <- function(values,params,condLevel,static=FALSE){
        ...
        if(static){
                p2<- ggplot(ccDF,aes(x=`Rubric Level`,y=`Co-occurrence Frequency`,fill=`Rubric Level`,label=Percentage))+
                        ...
                        ggtitle(paste("Rubric Levels Co-occurrent with",params$groupLevels[condLevel]))+
                        theme(text=element_text(family="Times"))
...
makeCondFreqPlot <- function(values,params,condLevel,static=FALSE){
        ...
        if(static){
                p2<- ggplot(condFreq,aes(x=`Rubric Level`,y=cpMinusOne,fill=`Rubric Level`,label=cpLabel))+
                        ...
                        ggtitle(paste("Rubric Levels Co-occurant with",rlNames[condLevel]))+
                        theme(text=element_text(family="Times"))
        }else{
                p2<- ggplot(condFreq,aes(x=`Rubric Level`,y=cpMinusOne,fill=`Rubric Level`,label=cpLabel))+
                        geom_bar(stat="identity",show.legend=FALSE)+
...
#--------------------------- Human scored functions ------------------------------------

# makeCooccurWDHS <- function(params,values,inset=0){
#         edgeScaleFactor <- 5      #scaling parameter for edges
#         nodeScaleFactor <- 5
#         
#         tmp <- as.data.frame(values$predictHS)
#         tmp <- tmp[,seq(1,ncol(tmp),2)]
#         
#         adjMat1 <- (t(tmp) %*% as.matrix(tmp)) #construct adjacency matrix
#         
...
#         
# }
# makeWebDiagramHS <- function(params,values,diagramLevel,minClassProb=0,inset=0){
#         #set only presence/absence
#         if(params$rubricType=="Typed"){
#                 tmp2 <- values$docTermMat[values$predictHS[[1]]$rubricLevel==diagramLevel & values$predictHS[[1]]$probability >= minClassProb,
#                                           params$oAbHS[[1]][1:params$maxCategoryN,diagramLevel]]
#         } else {
#                 tmp2 <- values$docTermMat[values$predictHS[[as.numeric(diagramLevel)]]$rubricLevel==1 &
#                                                   values$predictHS[[as.numeric(diagramLevel)]]$probability >= minClassProb,
#                                           params$oAbHS[[as.numeric(diagramLevel)]][[1]][1:params$maxCategoryN,2]]
#                 
#         }
#         tmp2$v[tmp2$v > 1] <- rep(1,length(tmp2$v[tmp2$v>1]))
#         tmp2 <- as.matrix(tmp2)       
...
# # generate binary scoring bar plot
# 
# makeBinaryScoringPlotHS <- function(params,static=FALSE){
#         tmpWD <-getwd()
#         setwd("/tmp")
#         if(!is.null(params$noneOfAboveHS)){
#                 # print(str(params$noneOfAbove))
#                 # print(str(params$groupFreq))
#                 figData <- rbind(params$groupFreqHS,params$noneOfAboveHS)
#                 figData[,1] <- factor(figData[,1],levels=rev(figData[,1]))
#                 # print(str(figData))
#         }else{
#                 figData <- params$groupFreqHS
#         }
#         if(static){
#                 # p <- ggplot(params$groupFreq,aes(x=`Rubric Level`,y=Responses,fill=`Rubric Level`))+
#                 p <- ggplot(figData,aes(x=`Rubric Level`,y=Responses,fill=`Rubric Level`))+
...
# }
# 
# makeTypedScoringPlotHS <- function(params,static=FALSE){
#         tmpDF <- cbind(params$groupFreqHS,data.frame(dummy=rep("dummy",nrow(params$groupFreqHS))))
#         Percentage <- paste(round(tmpDF$Documents/sum(tmpDF$Documents)*100),"%")
#         # cPosition <- sapply(seq_along(Percentage),function(x) sum(tmpDF[1:x,"Documents"])-0.5*tmpDF[x,"Documents"])
#         # tmpDF <- cbind(tmpDF,data.frame(Percentage=Percentage,cPosition=cPosition))
#         tmpDF <- cbind(tmpDF,data.frame(Percentage=Percentage))
...
# 
# #cooccurence plot
# makeCooccurFreqPlotHS <- function(values,params,condLevel,static=FALSE){
#         tmp <- as.data.frame(values$predictHS)
#         tmp <- tmp[seq(1,ncol(tmp),2)]
#         tmp[tmp>1] <- 1
#         
...
# 
# #make conditional frequency plot
# makeCondFreqPlotHS <- function(values,params,condLevel,static=FALSE){
#         tmpWD <-getwd()
#         setwd("/tmp")
#         rlNames <- params$groupFreqHS[,params$groupTypeHS]
#         condIndex <- values$predictHS[[condLevel]]$rubricLevel == 1
#         rlIndex <- 1:length(values$predictHS)
#         rlIndex <- rlIndex[rlIndex!=condLevel]
#         condFreq <- sapply(rlIndex,function(x) (sum(values$predictHS[[x]]$rubricLevel[condIndex])/
#                                                         sum(condIndex))/
#                                    (sum(values$predictHS[[x]]$rubricLevel)/
#                                             length(values$predictHS[[condLevel]]$rubricLevel))
#         )
#         
#         cpLabel <- round(condFreq,2)
#         maxCP <- max(abs(condFreq-1),na.rm=TRUE)
...
# }
# 
# applyFilterHS <- function(params,includeList,excludeList){
#         includeIndex <- params$groupLevels %in% names(params$wdMenuList)[as.numeric(includeList)]
#         excludeIndex <- params$groupLevels %in% names(params$wdMenuList)[as.numeric(excludeList)]
#         
#         includeIndex <- seq_along(includeIndex)[includeIndex]*2
#         excludeIndex <- seq_along(excludeIndex)[excludeIndex]*2
#         
#         outTable <- params$responseTableHS
#         for (index in includeIndex){
#                 outTable <- outTable[outTable[,index]==1,]
#         }
#         for(index in excludeIndex){
...
# }
# 
# makeHistogramPlotHS <- function(values,params,histogramLevel){
#         probIn <- data.frame(Probability=params$probTableHS[,histogramLevel])
#         g <- ggplot(probIn,aes(Probability))+
#                 geom_histogram(binwidth=0.05,na.rm=TRUE,fill=params$groupCols[histogramLevel])+
#                 xlim(0,1)+
#                 xlab("Score Probability")+
...
# }
# 
# makeImportantPlotHS <- function(params,firstLevel,secondLevel){
#         nBins <- (ncol(params$importanceTableHS)-1)/2
#         binNames <- names(params$wdMenuList)
#         if(secondLevel==0){
#                 datIn <- data.frame(Importance=params$importanceTableHS[,firstLevel+1],
#                                     Frequency=params$importanceTableHS[,firstLevel+nBins+1],
#                                     zeros=rep(0,nrow(params$importanceTableHS)),
#                                     Term=params$importanceTableHS$Term
#                 )
#                 
#                 g <- ggplot(datIn,aes(x=Importance,y=zeros,size=Frequency,z=Term))+
#                         geom_jitter(alpha=0.5,
...
#                                     color="blue")+
#                         scale_y_continuous(name="",labels="",breaks=c(),limits=c(-.1,.1))+
#                         scale_x_continuous(breaks=c(min(datIn$ImportanceHS),0,max(datIn$ImportanceHS)),
#                                            labels=c("Abscence \n Important","Neutral","Presence \n Important"),
#                                            name=paste("Term Importance: \n",binNames[firstLevel]))+
#                         geom_vline(xintercept = 0)+
#                         theme_minimal()+
...
#                 
#         }else{
#                 datIn <- data.frame(`First Term`=params$importanceTableHS[,firstLevel+1],
#                                     `Second Term`=params$importanceTableHS[,secondLevel+1],
#                                     Frequency=params$importanceTableHS[,firstLevel+nBins+1]+params$importanceTableHS[,secondLevel+nBins+1],
#                                     Term=params$importanceTableHS$Term
#                 )
#                 colnames(datIn) <- c("First Term","Second Term","Frequency","Term")
#                 
...
# }
# 
# makeVisNetworkPlot2HS <- function(values,params,width=NULL,arrows=NULL){
#         #creates visNetwork plot for data frame of binary input scores
#         tmp <- as.data.frame(values$predictHS)
#         scoresIn <- tmp[,seq(1,ncol(tmp),2)]
#         
#         nBins <- ncol(scoresIn)
#         binNames <- params$groupFreqHS[,params$groupType]
#         names(scoresIn) <- binNames
#         # binNames <- names(scoresIn)
#         
#         roundness <- max(c(0.05,0.28-0.02*nBins)) #determine curve of edges
