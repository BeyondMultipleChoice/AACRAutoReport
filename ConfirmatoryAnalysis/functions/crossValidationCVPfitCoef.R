...
#                 contTrain <- create_container(dmTrain,score[-testFolds[[i]]],trainSize=1:trainLength,virgin=FALSE)
                ...
                cvR <- apply(predictsValidation[,modIndex],2,function(x) 
                        cvRates(x,score[-testFolds[[i]]][-trainingPartition[[1]]],scoreLevels)) #returns columnwise TN,FN,FP,TP
                #select FPR for negative codes, TPR for postitve codes
                # cvR <- sapply(seq_along(modIndex),function(x) ifelse(predictsValidation[,modIndex[x]]==scoreLevels[2],cvR[4,x],cvR[2,x]))
                cvR <- sapply(seq_along(modIndex),function(x) ifelse(predictsValidation[,modIndex[x]]==scoreLevels[2],cvR[4,x],cvR[3,x]))
                cvRTest <- apply(predicts[,modIndex],2,function(x) 
                        cvRates(x,score[testFolds[[i]]],scoreLevels)) #returns columnwise TN,FN,FP,TP
                #select FPR for negative codes, TPR for postitve codes
                # cvRTest <- sapply(seq_along(modIndex),function(x) ifelse(predicts[,modIndex[x]]==scoreLevels[2],cvR[4,x],cvR[2,x]))
                cvRTest <- sapply(seq_along(modIndex),function(x) ifelse(predicts[,modIndex[x]]==scoreLevels[2],cvR[4,x],cvR[3,x]))
                
                        
                #construct ensemble prediction
                cvRTmp <-cbind(cvR,1-cvR)
                probsTmp <- cbind(tmpPV,1-tmpPV)
                cvRTest <-cbind(cvRTest,1-cvRTest)
                probsTest <- cbind(tmpPredicts,1-tmpPredicts)
                
                # ensembleModel <- optim(par=c(0.9,0.9),cvpErr,cvRIn=cvRTmp,probIn=probsTmp,
                #                        scoreIn=factor(score[-testFolds[[i]]][-trainingPartition[[1]]],levels = scoreLevels),
                #                        levelsIn=scoreLevels)
                
                # ensembleModel <- optim(par=c(1.0,1.0),cvpErr,cvRIn=cvRTmp,probIn=probsTmp,
                #                        scoreIn=score[-testFolds[[i]]][-trainingPartition[[1]]],
                #                        levelsIn=scoreLevels,control=list(fnscale=-1))

                
                paramGrid <- cbind(sort(rep(seq(0,3,.1),10)),rep(seq(0,3,.1),10))

                ensembleModel <- apply(paramGrid,1,function(x) cvpErr(coefIn = x,cvRIn=cvRTmp,probIn=probsTmp,
                                       scoreIn=score[-testFolds[[i]]][-trainingPartition[[1]]],
                                       levelsIn=scoreLevels))

                # print(table(ensembleModel))
                # print(c(min(ensembleModel),paramGrid[which.min(ensembleModel),]))

                # minIndex <- which(ensembleModel==min(ensembleModel))
                # if(length(minIndex)>1){
                #         par1 <- median(paramGrid[minIndex,1])
                #         par1Index <- paramGrid[minIndex,]
                #         par1Index <- par1Index[(which((par1Index[,1]-par1)==(min(par1Index[,1]-par1)))),2]
                #         par2 <- median(par1Index)
                #         coefsOut <- c(par1,par2)
                # }else{
                #         coefsOut <- paramGrid[which.min(ensembleModel),]
                # }
                maxIndex <- which(ensembleModel==max(ensembleModel))
                if(length(maxIndex)>1){
                        par1 <- median(paramGrid[maxIndex,1])
                        par1Index <- paramGrid[maxIndex,]
                        par1Index <- par1Index[(which((par1Index[,1]-par1)==(min(par1Index[,1]-par1)))),2]
                        par2 <- median(par1Index)
                        coefsOut <- c(par1,par2)
                }else{
                        coefsOut <- paramGrid[which.max(ensembleModel),]
                }

                print(paramGrid[maxIndex,])
                
                # print(c("final",ensembleModel$par))
                # ensemblePrediction <- cvpFunc(ensembleModel$par,cvRIn=cvRTest,probIn=probsTest,levelsIn=scoreLevels)
                
                # coefsOut <- ensembleModel$par
                
                print(c("final",coefsOut))
                ensemblePrediction <- cvpFunc(coefsOut,cvRIn=cvRTest,probIn=probsTest,levelsIn=scoreLevels)
                print(table(ensemblePrediction,score[testFolds[[i]]]))
                ...
                # tmpPred <- apply(ensemblePrediction,1,function(x) scoreLevels[x])
                
                print(table(ensemblePrediction))
                #tmpPred <- factor(ensemblePrediction,levels=scoreLevels)
                tmpPred <- ensemblePrediction
                print(table(tmpPred))
                ...
                print(table(factor(predictOut,scoreLevels)))
        ...
        print(table(predictOut2))
        print(table(factor(predictOut2[completeIndex],scoreLevels)))
