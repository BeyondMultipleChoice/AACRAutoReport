...
removeHTMLtags <- function(x){
        # tmp <- sapply(x,function(x) bracketX(x,"angle"),USE.NAMES = FALSE)
        tmp <- gsub("<.*?>", "", x)
        tmp
}
...
stemReplaceNAs <- function(stemmedText,completedText,dtmIn,datIn,question){
        ...
        for(pWord in problemWords){
                # tmpIndex <- which(as.vector(dtmIn[,pWord]==1))[1]
                ...
                responseDistVector <- adist(pWord,problemResponse,costs=list("i"=1,"d"=1,"s"=2)) #calculate edit distances
...
preprocessData <- function(datIn,question,removeStopwords="DEFAULT",stemming=TRUE,numberFormat="remove",preserveSymbol=""){
        ...
        if(length(removeStopwords)==1 & removeStopwords[1]=="DEFAULT"){
                tmp <- removeWords(tmp,localStopwords)
        }else if(removeStopwords[1]!="NONE"){
                tmp <- removeWords(tmp,removeStopwords)
        }
...
tryTrainModels2 <- function(dtmIn,scoreIn,algorithms){
        outModels <- list()
        for(alg in algorithms){
                print(paste("algorithm",alg))
                runTime <- system.time(modelTmp <- try(caret::train(x=dtmIn,y=scoreIn,method=alg)))
                # runTime <- system.time(modelTmp <- try(caret::train(ScoreIn ~ .,data=cbind(data.frame(ScoreIn=scoreIn),dtmIn),method=alg)))
                timeDF <- data.frame(alogrithm=alg,
                                     n=length(scoreIn),
                                     features=ncol(dtmIn),
                                     userSelf=runTime[1],
                                     sysSelf=runTime[2],
                                     ...
                                     userChild=runTime[4],
                                     sysChild=runTime[5])
                print(paste(alg,paste(runTime,collapse=" ")))
                # data.table::fwrite(timeDF,"/home/msteele/AACR/caretTesting/algorithmTimeInd2.csv",append=TRUE)
                
                if(class(modelTmp)!="try-error"){
                        print(paste(alg,"passes"))
                        outModels[[alg]] <- modelTmp
                }else{
                        print(paste(alg,"fails"))
                }
        }
        outModels
}
...
predictFromModels <- function(modelsIn,testData){
        outPrediction <- list()
        for (modTmp in modelsIn){
                predTmp <- predict(modTmp,testData,type="prob")
                predTmp <- cbind(predTmp,data.frame(predictedScore=colnames(predTmp)[max.col(predTmp,ties.method = "first")]))
                outPrediction[[modTmp$method]] <- predTmp
        }
        outPrediction
}
tryPredictFromModels <- function(modelsIn,testData){
        outPrediction <- list()
        for (modTmp in modelsIn){
                predTmp <- try(predict(modTmp,testData,type="prob"))
                if(class(predTmp)!="try-error"){
                        print(paste(modTmp$method,"pred passes"))
                        predTmp <- cbind(predTmp,data.frame(predictedScore=colnames(predTmp)[max.col(predTmp,ties.method = "first")]))
                        outPrediction[[modTmp$method]] <- predTmp
                }else{
                        print(paste(modTmp$method,"pred fails"))
...
cvpFunc <- function(coefIn,cvRIn,probIn,levelsIn){
        print(c("func call",coefIn))
        nc <- ncol(cvRIn)
        pTmp <- ifelse(rowSums(cvRIn[,1:(nc/2)]^coefIn[1]*probIn[,1:(nc/2)]^coefIn[2])>rowSums(cvRIn[,(nc/2+1):nc]^coefIn[1]*probIn[,(nc/2+1):nc]^coefIn[2]),
                       1,0)
        pTmp
}
cvpErr <- function(coefIn,cvRIn,probIn,scoreIn,levelsIn){
        pTmp <- cvpFunc(coefIn,cvRIn,probIn,levelsIn)
        diffOut <- confusionMatrix(factor(pTmp,levelsIn),factor(scoreIn,levelsIn))$overall["Kappa"]
        print(c("metric",diffOut))
        diffOut
}
