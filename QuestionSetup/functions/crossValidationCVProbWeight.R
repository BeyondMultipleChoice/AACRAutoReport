...
for(scoreColumn in scoreColumnVector){
        ...
        predDF <- as.data.frame(predDF[order(unlist(testFolds)),]) #reorder intermediate predictions to match input ordering
        ...
        #cv weighing, determine true/false positive/negative rates
        cvWTmp <- apply(predDF[,modColIndex],2,function(x) cvRates(x,score,scoreLevels))[3:4,] #returns columnwise FP & TP rates
        stackingModel <- c(stackingModel,list(cvWTmp))
...
resave(stackingModel,file=modFileOut)
