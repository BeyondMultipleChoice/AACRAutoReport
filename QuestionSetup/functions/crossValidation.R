...
if (input$rubricType=="Typed"){
        positiveTmp <- NULL
}else if(is.null(input$positiveCode)){
        positiveTmp <- "1" 
}else{
        positiveTmp <- input$positiveCode
}
...
for(scoreColumn in scoreColumnVector){
        ...
        for(i in 1:nFolds){
                ...
                models <- tryTrainModels(contTrain,algorithms=rownames(values$modNameDFList[[rubricLevelIndex]])) #use only models used in stored ensemble object
...
#                 predOut$probability <- apply(cbind(classTmp,predOut$rubricLevel),1,estimateProb)
        ...
        # confMat <-confusionMatrix(score,predictOut2)
        # print(c(scoreLevels,positiveTmp))
        confMat <-confusionMatrix(factor(predictOut2[completeIndex],scoreLevels),factor(score,scoreLevels),positive = positiveTmp)
        confusionMatrixOut[[length(confusionMatrixOut)+1]] <- confMat 

        #agreement/probability data frame
        disagreeIndex <- predictOut2[completeIndex]!=score
        if(input$rubricType=="Typed"){
                typeTmp <- score[disagreeIndex]        
        }else{
                typeTmp <- rep(rubricLevelNames[which(scoreColumn==scoreColumnVector)],sum(disagreeIndex))
        }
...
cvFileOut <- file.path(outputDir,paste0(questionName,"CrossValidation",params$version,".Rdata"))
save(confusionMatrixOut,scoreLength,histDF,file=cvFileOut)
