...
#                 contTrain <- create_container(dmTrain,score[-testFolds[[i]]],trainSize=1:trainLength,virgin=FALSE)
                ...
                #construct ensemble prediction, lasso plr
...
#                 ensemblePrediction <- predict(ensembleModel,newdata=tmpPredicts,type="prob")
                ...
                print(table(ensembleTrain$Score))

                l1Out <- try(optL1(Score ~ .,
                               data=ensembleTrain,
                               model="logistic",
                               fold=10,
                               maxiter=100))
                if(class(l1Out)=="try-error"){ #check for failed lambda fits
                        l1Out$lambda <- 1
                }
                print("hi")
                ensembleModel <- penalized(Score ~ .,
                               data=ensembleTrain,
                               model="logistic",
                               lambda1=l1Out$lambda,
                               maxiter = 100)
                print("there")
                tmp <- predict(ensembleModel,tmpPredicts)
                print("!")
                ensemblePrediction <- cbind(1-tmp,tmp)
