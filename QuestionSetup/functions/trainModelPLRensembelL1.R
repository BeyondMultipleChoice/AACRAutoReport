...
for(i in seq_along(scoreColumnVector)){
        ...
        print(trainLength)
        print(table(datInTmp[completeIndex,scoreColumnVector[i]]),useNA="ifany")
        #create container
...
# groupLevels <- as.character(values$hot[,"Rubric Level"])
# majorVersionDescription <- params$majorVersionDescription
# numMinorVersions <- params$numMinorVersions
...
save(models,questionDictionary,completedNgrams,preproc,file=modFileOut)
...
# save(questionText,rubricType,groupLevels,questionRubric,codeList,file=metaFileOut)
# 
# #update question list
# load(file.path("..","AutoReport","questionData","questionList.Rdata")) #load question list
